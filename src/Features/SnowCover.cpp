...
void SnowCover::BSLightingShader_Setup(RE::BSRenderPass* a_pass)
{
	...
	if (blacklist.contains(FormIdParser::fnv_hash(name))) {
		state->permutationData.ExtraFeatureDescriptor |= (uint)State::ExtraFeatureDescriptors::NoSnow;
	...
	if (tint_blacklist.contains(FormIdParser::fnv_hash(name)))
		state->permutationData.ExtraFeatureDescriptor |= (uint)State::ExtraFeatureDescriptors::NoTint;
	else
		state->permutationData.ExtraFeatureDescriptor &= ~(uint)State::ExtraFeatureDescriptors::NoTint;
}
